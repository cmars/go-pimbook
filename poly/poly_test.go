...
func TestAdd(t *testing.T) {
	t.Run("add 2 polys, both degree=2", func(t *testing.T) {
		...
		c.Assert(sum.Coeff(), ratSliceEquals, poly.NewInt(4, 7, 8).Coeff())
	})
...
func TestString(t *testing.T) {
	t.Run("strings", func(t *testing.T) {
		c := qt.New(t)
		c.Assert(poly.NewInt(7).String(), qt.Equals, "7")
		c.Assert(poly.NewInt(7, 2).String(), qt.Equals, "7+2x")
		c.Assert(poly.NewInt(7, 2, 9).String(), qt.Equals, "7+2x+9x^2")
		c.Assert(poly.NewInt(-7, -2, -9).String(), qt.Equals, "-7-2x-9x^2")
		c.Assert(poly.NewInt(7, 0, 9).String(), qt.Equals, "7+9x^2")
		c.Assert(poly.NewInt(0, 0, 9).String(), qt.Equals, "9x^2")
	})
}
...
func TestMul(t *testing.T) {
	t.Run("multiply 2 polys, both degree=2", func(t *testing.T) {
		...
		c.Assert(prod.Coeff(), ratSliceEquals, poly.NewInt(7, 19, 20, 11, 3).Coeff())
	})
}
...
func TestInterpolate(t *testing.T) {
	t.Run("interpolate poly for given points", func(t *testing.T) {
		c := qt.New(t)
		p, err := poly.Interpolate(poly.NewPointInt(1, 1))
		c.Assert(err, qt.IsNil)
		c.Assert(p.Coeff(), ratSliceEquals, poly.NewInt(1).Coeff())

		p, err = poly.Interpolate(poly.NewPointInt(1, 1), poly.NewPointInt(2, 0))
		c.Assert(err, qt.IsNil)
		c.Assert(p.Coeff(), ratSliceEquals, poly.NewInt(2, -1).Coeff())

		p, err = poly.Interpolate(poly.NewPointInt(1, 1), poly.NewPointInt(2, 4), poly.NewPointInt(7, 9))
		c.Assert(err, qt.IsNil)
		c.Assert(p.Coeff(), ratSliceEquals, poly.New(
			big.NewRat(-8, 3), big.NewRat(4, 1), big.NewRat(-1, 3)).Coeff())
	})
}
